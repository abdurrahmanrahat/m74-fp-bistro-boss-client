/**
 * -----------------------------------
 *  Steps for stripe and react stripe
 * ------------------------------------
 * 1) install stripe and react stripe js
 * 2) create a checkOutForm with CardElement, cardElement contains (card number,        expiration date, cvs, zip code)
 * 3) create account on stripe and get the publishable key (pk).
 * 4) get card information
 * 5) create a payment method
 * 6) use text card to test payment 
 * 7) on the server side, install stripe: npm install --save stripe
 * 8) create a payment intent api with payment method types: ['card'] 
 * 8.5) make sure you send the amount in sends: const amount = price * 100;
 * 9) call payment intent api to get client secret and store it in a state.
 * 10) use confirm card payment api with client secret and card info.
 * 11) display confirm card error and success
 * 12) do things after payment --> 
 * 
*/